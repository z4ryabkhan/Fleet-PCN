...
const TOGGLE_BTN =
  "min-h-11 flex-1 rounded-lg py-2 font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-planal-brand";
...
export function SignUpForm({
  ...
  draftToken?: string;
  defaultAccountType?: "fleet" | "individual";
}) {
  const [state, formAction, pending] = useActionState(signUpAction, initialState);
  const [accountType, setAccountType] = useState<"fleet" | "individual">(defaultAccountType);

  return (
    <form action={formAction} className="space-y-5">
      <input type="hidden" name="accountType" value={accountType} />
      {draftToken && <input type="hidden" name="draftToken" value={draftToken} />}
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-planal-ink-muted">At least 8 characters.</p>
      </div>
      ...
      <button
        type="submit"
        disabled={pending}
        className="min-h-11 w-full rounded-2xl bg-planal-brand px-4 py-3 font-semibold text-white transition-opacity hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-planal-brand-dark focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60"
      >
