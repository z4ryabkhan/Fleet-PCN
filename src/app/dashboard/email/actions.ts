...
// One-click revoke — Part 4 rule 6. Revokes the token at Google (so it
// stops working immediately, not just locally) and marks the connection
// row 'revoked' rather than deleting it, so there's a durable record of
// what was ever connected.
export async function revokeConnectionAction(
  ...
  try {
    ...
    // Revoking at Google is best-effort — even if it fails (token already
    // invalid, network issue), we still mark the connection revoked
    // locally so it stops being used for scanning.
